...
we implement the ability to add more cameras, so in the GUI we add button add camera. so to show that we add 2 cameras: camera 0 and camera 1 and we can see the view volume different from the other cameras: (we do different camera transformation for camera 0 and camera 1) image when we choose camera 0 : ![alt text]()
image when we choose camera 1 : ![alt text]()
